...
#Primero verifica si el usuario esta en la sesión, si no está lo manda al login, y si esta renderiza el index
    context={
        "selfuser": User.objects.get(id=request.session['user']),
        "usuarios": User.objects.all(),
        "poked" : Pokes.objects.all().values('userpoked','userpoke__name').annotate(Count('userpoke')).filter(userpoked=User.objects.get(id=request.session['user'])),
        "pokeHistory": User.objects.annotate(Count('poked')).exclude(id=request.session['user'])
    }
...
def poke(request): 
    print(request.POST['targetuser'])
    print(request.session['user'])
    poke= Pokes.objects.create(
        userpoke= User.objects.get(id=request.session['user']),
        userpoked= User.objects.get(id=request.POST['targetuser'])
    )
    return redirect("/")